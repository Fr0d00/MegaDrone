...
void regulateXY(GyroData* data, MotorData* mData, RadioData* rData){
    float uX = _calc_pid(data->x - float(map((rData->x - 127), -128, 127, -15, 15)), Pk);
    float uY = _calc_pid(data->y - float(map((rData->y - 127), -128, 127, -15, 15)), Pk);
